    ...
    for subject in tqdm(data_dict.keys(), desc="Loading dataset for YOLO conversion."):
        ...
        for sample in data_dict[subject].keys():
            ...
            image_mask_pairs.append(
                (image_name, img, os.path.join(processed_masks_dir, label_name))
            )
...
def preprocess_data_coco(
    ...
    train_annotations_file = os.path.join(
        processed_annotations_dir, "json_annotation_train.json"
    )
    val_annotations_file = os.path.join(
        processed_annotations_dir, "json_annotation_val.json"
    )
    test_annotations_file = os.path.join(
        processed_annotations_dir, "json_annotation_test.json"
    )
    ...
    for subject in tqdm(data_dict.keys(), desc="Loading dataset for COCO conversion."):
        ...
        for sample in data_dict[subject].keys():
            ...
            image_mask_pairs.append(
                (image_name, img, image_info, axon_myelin_annotations)
            )
    ...
    save_split(
        [entry for entry in image_mask_pairs if entry[0] in data_split["val"]],
        val_images_dir,
        val_annotations,
    )
    save_split(
        [entry for entry in image_mask_pairs if entry[0] in data_split["test"]],
        test_images_dir,
        test_annotations,
    )
